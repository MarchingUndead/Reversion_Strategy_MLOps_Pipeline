"""Register the top run per position from the grid-search experiment.

For each position (0, 1, 2), finds the run with the highest metrics.f1_macro
and registers all three heads (classifier, duration, revert) from that run
under names:

    reversion-classifier-pos{pos}
    reversion-duration-pos{pos}
    reversion-revert-pos{pos}

Optionally promotes the new version to Production with --promote.

Usage:
    ./.venv/Scripts/python.exe scripts/register_best.py
    ./.venv/Scripts/python.exe scripts/register_best.py --promote
    ./.venv/Scripts/python.exe scripts/register_best.py --experiment reversion-grid --metric f1_macro
    ./.venv/Scripts/python.exe scripts/register_best.py --position 2 --promote
"""
...
def main():
    ...
    for pos in positions:
        filter_string = f"tags.position = '{pos}'"
        if args.prefer_rf:
        ...
        if runs.empty:
            print(f"pos {pos}: no runs matching tags.position = '{pos}' — skipped")
            continue
        ...
        print(f"\npos {pos}: winner model={model}  {args.metric}={mvalue:.4f}  "
              f"params={params}  run_id={run_id}")

        for head in HEADS:
            name = f"reversion-{head}-pos{pos}"
            try:
